...
def get_bot_response(user_text, mode):
    ...
    if not active_key:
        active_key = st.secrets.get("POLLINATIONS_API_KEY", "").strip()
        
    headers = {}
    if active_key and active_key != "sk-baymax":
    ...
    for fallback_model in models_to_try:
        for attempt in range(2):
            try:
                ...
                with requests.get(url, headers=headers, stream=True, timeout=8.0) as response:
    ...
    if "429" in last_error or "queue full" in last_error.lower() or "503" in last_error:
        yield "My servers are currently experiencing heavy traffic! 🚦 Please wait a few moments and ask me again.\n\n*Tip: Get a 100% free key from **https://enter.pollinations.ai** and paste it in the sidebar to completely bypass all rate limits!*"
    else:
